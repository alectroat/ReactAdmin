...
namespace API_Project.Concrete
{
    public class LoginRepository : ILoginRepository, IDisposable
    {
        ...
        Registration ILoginRepository.AuthenticUser(Registration Registration)
        {
            return _Context.Registrations.Where(p => p.UserName == Registration.UserName && p.Password == Registration.Password).FirstOrDefault<Registration>();
        }
        public void Dispose()
        {
            _Context.Dispose();
